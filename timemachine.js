(function(name, definition) {
  ...
  var DateOptionsStack = [],
    ...
    Timemachine = {
      ...
      config: function(options, shouldPush) {
        if(shouldPush === undefined) {
            shouldPush = true;
        }
        if(shouldPush) {
            DateOptionsStack.push(options);
        }
      ...
      reset: function() {
        ...
        this.keepTime = false;
        this.difference = 0;
        DateOptionsStack.pop();
        if(!DateOptionsStack.length) {
            Date = OriginalDate;
            Date.prototype = OriginalDate.prototype;
        }
        else {
            var o = DateOptionsStack.slice(-1)[0];
            this.config(o, false);
        }
      },
